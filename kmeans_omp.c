...
int main(int argc, char **argv) {
    ...
    while (nchanges > 0 && curiter <= MAXITER) { // Loop until convergence
        ...
        #pragma omp parallel
        {
            //creating local copy of clust->feats
            float feats[nclust * data->dim];
            for(int i = 0; i < nclust*data->dim; i++){
                feats[i] = 0.0;
            }
            #pragma omp for
            for (int i = 0; i < data->ndata; i++) { // Sum up data in each cluster
                int c = data->assigns[i];
                    for (int j = 0; j < clust->dim; j++) {
                        feats[c * clust->dim + j] += data->features[i * clust->dim + j];
                    }
            }
            
            //adding features in local features array to the global features array
            #pragma omp critical
            for(int i = 0; i < nclust * clust->dim; i++){
                clust->features[i] += feats[i];
            }
        }
        ...
        for (int i = 0; i < clust->nclust; i++) { // Divide by ndatas of data to get mean of cluster center
            if (clust->counts[i] > 0) {
                for (int j = 0; j < clust->dim; j++) {
                    clust->features[i * clust->dim + j] = clust->features[i * clust->dim + j] / clust->counts[i];
                }
            }
        }
        ...
        #pragma omp parallel
        {   
            //creating local copy of clust->counts in each thread
            float counts[nclust];
            for(int i = 0; i < nclust; i++){
                counts[i] = 0;
            }
            ...
            //adding counts from local counts array to the global counts array
            #pragma omp critical
            for(int i = 0; i < nclust; i++){
                clust->counts[i] += counts[i];
            }
        }
